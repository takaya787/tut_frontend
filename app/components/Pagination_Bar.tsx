...
export const Pagination_Bar: React.FC<PaginationPropType> = ({ pageState, setPageState }) => {
  ...
  const ClickNumber = (number: number) => { setPageState(Object.assign({ ...pageState }, { currentPage: number })) }
    ...
    return (
      <>
        { pageNumber === currentPage ? (
          ...
          <Pagination.Item key={pageNumber} onClick={() => ClickNumber(pageNumber)}>{pageNumber}</Pagination.Item>
        )}
      </>
    )
  }
  ...
  // Paginationの全体の数
  const Pagination_Numbers = ({ totalPage, currentPage }: { totalPage: number, currentPage: number }): React.ReactElement => {
