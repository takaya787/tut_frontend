      ...
      <Container>
        <Row>
                  ...
                  <>
                    <Col sm={4}>
                      <External_Image alt="User icon"
                        src={followers_state.user.gravator_url} width={70} height={70}
                      />
                    </Col>
                    <Col sm={8}>
          ...
          <Col md={7}>
            ...
            {user_data && has_user_key() && id_checker(Number(id), user_data.user.id) && relationships_data && has_followers_key() && (
              <UserFollowList follows={relationships_data.relationships.followers} />
            )}
            {/* user_idとprops_idが異ればstateのデータを使用 */}
            {user_data && has_user_key() && !id_checker(Number(id), user_data.user.id) && followers_state && (
